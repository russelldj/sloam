...
namespace seg
{
  ...
  void Segmentation::_startONNXSession(const std::string sessionName, const std::string modelFilePath, bool useCUDA, size_t numThreads)
  {
    ...
    std::cout << "Trying to setup Onyx with use_cuda = " << useCUDA << std::endl;
    ...
    if (useCUDA)
    {
      ...
      std::cout << "Onyx is using CUDA" << std::endl;
    }
    std::cout << "Onyx is setup" << std::endl;
  ...
  void Segmentation::_makeTensor(std::vector<std::vector<float>> &projected_data, std::vector<float> &tensor, std::vector<size_t> &invalid_idxs)
  {
    ...
    for (uint32_t pixel_id = 0; pixel_id < projected_data.size(); pixel_id++)
    {
      ...
      all_zeros = std::all_of(projected_data[pixel_id].begin(), projected_data[pixel_id].end(), [](int i)
                              { return ((i == 0.0f) || (isnan(i))); });
      if (all_zeros)
      {
                               ...
                               bool dense)
  {
    ...
    std::cout << "about to test assert" << std::endl;
  ...
  void Segmentation::run(const Cloud::Ptr cloud, cv::Mat &maskImg)
  {
    ...
    // return; // Creating ground cloud
    //  Somewhere between these two returns there's a memory mangement issue
    _session->Run(Ort::RunOptions{nullptr}, _inputNames.data(),
                  inputTensors.data(), 1, _outputNames.data(),
                  outputTensors.data(), 1);
    std::cout << outputTensors.at(0) << std::endl;
    // return; // double free or corruption (!prev)
    if (_verbose)
    {
      _timer.toc();
    }
    ...
    std::cout << "Done running run" << std::endl;
